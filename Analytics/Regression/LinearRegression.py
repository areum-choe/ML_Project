...
def linear_regression_run(request_json):
    ...
    try:
        ...
        if len(df.columns) == col_maxcnt:
            ...
            df = df.iloc[:, [0, 1]]
            # df = df.dropna()
            input_independent = df_input_independent[0]
            input_yn = True
        else:
            input_yn = False
        ...
        result_Image = common.Path_Prj_Main() + 'Data/Image/ReportResult/' + 'LinearImg_' + common.Regexp_OnlyNumberbyDate() + '.png'
        # 회귀분석 시각화
        font_size = 15
        ...
        # 예측입력값이 있을 경우
        if input_yn and round(fit.predict(exog=dict(INDEPEN=[int(input_independent)])), number_cut)[0] > 0:
            input_val = [round(fit.predict(exog=dict(INDEPEN=[int(input_independent)])), number_cut)[0]]
        ...
        # 분석결과 리포트 생성
        fileNameArr = [word.CreateReport_LinearRegression(return_df, result_Image)]
        return_df = pd.concat([return_df, pd.DataFrame(fileNameArr, columns=["FILENAME"])], axis=1)
        print(return_df.SUMMARY[0])
