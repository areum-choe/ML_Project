...
def lassoRegression_run(request_json):
    try:
        ...
        # 실제값 & 예측값 합치기
        result_lasso = pd.concat([y_test, pd.DataFrame(y_predict, columns=['예측값'])], axis=1)

        # 오차값 합치기
        result_lasso = pd.concat([result_lasso, pd.DataFrame(result_lasso.실제값 - result_lasso.예측값, columns=['오차값'])], axis=1)

        # 상관관계
        corr = pfm.Correlation(df)

        # 합치기
        result_lasso = pd.concat([corr, result_lasso], axis=1)
        ...
        result_Image = common.Path_Prj_Main() + 'Data/Image/ReportResult/' + 'RidgeImg_' + common.Regexp_OnlyNumberbyDate() + '.png'
        ...
        # 합치기
        result_lasso = pd.concat([result_lasso, pd.DataFrame(alpha)], axis=1)
        result_lasso = pd.concat([result_lasso, train_score_alpha], axis=1)
        result_lasso = pd.concat([result_lasso, test_score_alpha], axis=1)

        # 평가지표
        eval_all = pfm.eval_all(y_test, pd.DataFrame(y_predict))
        result_lasso = pd.concat([result_lasso, pd.DataFrame(eval_all, columns=['평가지표'])], axis=1)
        ...
        # 합치기
        result_lasso = pd.concat([result_lasso, feature_importance], axis=1)

        result_lasso = result_lasso.round({'예측값': 3, '오차값': 3, 'train_score_alpha': 3, 'test_score_alpha': 3, 'importances': 3})

        # 분석결과 리포트 생성
        fileNameArr = [word.CreateReport_RidgeRegression(result_lasso, result_Image)]
        result_lasso = pd.concat([result_lasso, pd.DataFrame(fileNameArr, columns=["FILENAME"])], axis=1)
