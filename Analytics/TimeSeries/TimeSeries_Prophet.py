...
import CommonLib.Report.WordManager as TimeSeries_Prophet2
import pandas as pd
...
def time_series_prophet_run(request_json):
    """
    ...
    try:
        ...
        # 평가지표
        real_y = result_df.y.head(len(df))
        pred_y = result_df.yhat.head(len(df))
        eval_all = pfm.eval_all(real_y, pred_y)
        result_df = pd.concat([result_df, pd.DataFrame(eval_all, columns=['평가지표'])], axis=1)
        ...
        # 결과 이미지 생성
        result_Image = common.Path_Prj_Main() + 'Data/Image/ReportResult/' + 'ProphetImg_' + common.Regexp_OnlyNumberbyDate() + '.png'
        ...
        plt.xlabel(word.timeSeries_Xlabel, fontsize=10)
        plt.ylabel(word.timeSeries_Ylabel, fontsize=10)
        plt.axvspan(len(result_df['y'])-31, len(result_df['y']), facecolor='grey', edgecolor='grey', alpha=0.5, hatch='///')
        plt.savefig(result_Image)

        # 분석결과 리포트 생성
        fileNameArr = [word.CreateReport_TimeSeries_Prophet(result_df, result_Image)]
